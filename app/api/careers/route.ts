...
function rateLimited(key: string) { const now = Date.now(); const current = requests.get(key); if (!current || current.resetAt <= now) { requests.set(key, { count: 1, resetAt: now + 60 * 60 * 1000 }); return false }; if (current.count >= 3) return true; current.count += 1; return false }
...
export async function POST(request: Request) {
  try {
    ...
    const name = text(form.get('name'), 120), email = text(form.get('email'), 254), phone = text(form.get('phone'), 40), role = text(form.get('role'), 80), portfolio = text(form.get('portfolio'), 500), linkedin = text(form.get('linkedin'), 500), github = text(form.get('github'), 500), why = text(form.get('why'), 3000), availability = text(form.get('availability'), 500)
    ...
    const roleNames: Record<string, string> = { 'website-developer': 'Website Developer', 'ui-ux-designer': 'Website / UI/UX Designer', 'video-editor': 'Video Editor', 'social-media-manager': 'Social Media Manager' }
    const roleName = roleNames[role]
    const subject = `New Career Application - ${roleName} - ${name}`
    const rows = [['Name', name], ['Email', email], ['Phone / WhatsApp', phone], ['Role', roleName], ['Portfolio', portfolio], ['LinkedIn', linkedin], ['GitHub', github], ['Availability', availability], ['Why SiteCrafters', why]].filter(([, value]) => value).map(([label, value]) => `<tr><td style="padding:9px 0;color:#999;font-size:12px;width:150px;vertical-align:top">${escapeHtml(label)}</td><td style="padding:9px 0;color:#f7f4f1;font-size:14px;line-height:1.5">${escapeHtml(value)}</td></tr>`).join('')
    ...
    const payload = { from, to: [inbox], reply_to: email, subject, html, text: `Career application\n\nName: ${name}\nEmail: ${email}\nRole: ${roleName}\nPhone: ${phone || 'Not provided'}\nPortfolio: ${portfolio || 'Not provided'}\nLinkedIn: ${linkedin || 'Not provided'}\nGitHub: ${github || 'Not provided'}\nAvailability: ${availability}\n\nWhy SiteCrafters:\n${why}`, ...(attachment ? { attachments: [attachment] } : {}) }
    ...
    if (!resendResponse.ok) { console.error('Resend career error', await resendResponse.text()); return Response.json({ success: false, message: 'Unable to submit application right now.' }, { status: 502 }) }
    return Response.json({ success: true, message: 'Application submitted successfully.' })
  } catch (error) { console.error('Career application error', error); return Response.json({ success: false, message: 'Unable to submit application right now.' }, { status: 500 }) }
}
