...
export async function POST(request: Request) {
  try {
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [inbox], reply_to: email, subject, html, text: textContent }) })
    if (!resendResponse.ok) { console.error('Resend enquiry error', await resendResponse.text()); return Response.json({ success: false, message: 'Unable to send enquiry right now.' }, { status: 502 }) }
